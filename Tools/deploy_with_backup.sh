...
# --- Guard v2: forbid any data/Data dirs inside App (golden rule) ---
guard_no_app_data_v2(){
  APP="/srv/luckypack/App"; ROOT="/srv/luckypack"
  BAD=$(find "$APP" -type d \( -iname data -o -iname Data \) -print || true)
  RED="\033[1;31m"; YEL="\033[1;33m"; GRN="\033[1;32m"; BLD="\033[1m"; RST="\033[0m"
  if [ -n "$BAD" ]; then
    echo -e "${RED}🚫 DEPLOY BLOCKED${RST} — запрещены каталоги \"data\" внутри App"
    echo
    echo -e "${YEL}Найдены:${RST}"
    echo "$BAD" | sed "s/^/  • /"
    echo
    echo -e "Разрешён только: ${BLD}$ROOT/data${RST}"
    echo -e "Подчистить можно так: ${BLD}find \"$APP\" -type d \\( -iname data -o -iname Data \\) -empty -print -delete${RST}"
    exit 1
  else
    echo -e "${GRN}✅ Guard OK${RST}: внутри App нет каталогов \"data\""
  fi
}
# guard_no_app_data_v2 (disabled by v3)
...
# bash -lc "/srv/luckypack/App/Tools/deploy_with_backup.sh \"deploy: $(date +%F\ %T) — no-cache build; commit+push; backup; guard v3\""
# Вставить однострочный аргумент для коммита